...
exports.getStudents = (req,res,next) => {
    ...
    .then((result) => {
        console.log(result);
        res.render('addStudent',{prods:result,isAuthenticated:req.session.isLoggedIn})
      })
...
exports.showstudent = (req,res,next) => {
    ...
    .then((result) => {
     console.log(result);
     console.log(result);
        res.render('showStudents',{prods:result,isAuthenticated:req.session.isLoggedIn})
...
exports.deleteStudent = (req,res,next) => {
       ...
       Student.findOneAndDelete({_id: req.body.id })
       .then(result => {
        console.log(result);
        res.redirect('/');
    })
...
exports.postcertificate =  (req,res,next) => {
    let id = req.params.id;
    certificate.find({name:id})
    .then(certiId => {
        console.log(certiId[0]._id);
        Student.find({certificateid:certiId[0]._id})
        .populate('certificateid',"-_id")
        .then((result) => {
        console.log(result);
        res.json(result)
